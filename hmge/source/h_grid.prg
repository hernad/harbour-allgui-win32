...
*-----------------------------------------------------------------------------*
FUNCTION _DefineGrid ( ControlName, ParentFormName, x, y, w, h, aHeaders, aWidths, aRows, ;
      value, fontname, fontsize, tooltip, change, dblclick, aHeadClick, gotfocus, lostfocus, ;
      nogrid, aImage, aJust, break, HelpId , bold, italic, underline, strikeout, ownerdata, ;
      ondispinfo, itemcount, editable, dynamicforecolor, dynamicbackcolor, multiselect, ;
      editcontrols, backcolor, fontcolor, nId, columnvalid, columnwhen, validmessages, ;
      showheaders, aImageHeader, NoTabStop, celled, lCheckboxes, lockcolumns, ;
      OnCheckBoxClicked, doublebuffer, nosortheaders, columnsort, aWidthLimits, ondragitems, bInit )
*-----------------------------------------------------------------------------*
         ...
         IF ISARRAY( aEditControls [ci] )
            ...
            IF Len( aEditControls [ci] ) >= 1
               ...
               IF Len( aEditControls [ci] ) == 2 .AND. AEC == 'DYNAMIC'
                  ...
                  IF ISARRAY( aEdit ) .AND. Len( aEdit ) >= 1
