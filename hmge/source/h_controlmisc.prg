...
#ifdef _HMG_COMPAT_
            ci := iif( Len ( _HMG_aControlBkColor [i] ) > 0, 1, 0 )
            aTemp := AClone ( Value )
            AEval( aTemp, {|x, i| iif( i > ci, aTemp [i] := iif( ISCHARACTER( x ), x, hb_ValToStr( x ) ), ) } )
...
FUNCTION InputWindow ( cTitle, aLabels, aValues, aFormats, ;
      nRow, nCol, lCenterWindow, aButOKCancelCaptions, nLabelWidth, nControlWidth )
*-----------------------------------------------------------------------------*
...
*-----------------------------------------------------------------------------*
   ...
   FOR EACH h IN _HMG_aControlParentHandles
      ...
      IF h == hWnd .AND. ;
         iif( cUserType == "ALL", .T., GetUserControlType ( _HMG_aControlNames [i], GetParentFormName( i ) ) == cUserType )
