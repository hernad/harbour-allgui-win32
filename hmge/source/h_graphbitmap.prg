...
FUNCTION HMG_PieGraph( nWidth, nHeight, series, aname, colors, ctitle, aTitleColor, depth, l3d, lxval, lsleg, lNoborder, cPicture, placement )
   ...
   IF Len( ctitle ) > 0
      DrawTextInBitmap( hDC, fromrow + 12, iif( Len( ctitle ) * 8 > ( tocol - fromcol ), fromcol, Int( ( ( tocol - fromcol ) - ( Len( AllTrim( ctitle ) ) * 8 ) ) / 2 ) + fromcol ), ctitle, _HMG_DefaultFontName, _HMG_DefaultFontSize + 3, aTitleColor )
