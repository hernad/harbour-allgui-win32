...
Procedure Main( cDBFName )
  ...
  IF Empty( cDBFPath := cFilePath( cDBFName ) )
    cDBFPath := GetStartupFolder()
  ENDIF
  cDBFPath += "\"

  // Set default RDD and open a data file
  IF Empty( File( cDBFPath + cFileNoExt( cDBFName ) + ".fpt" ) )
  ...
  DEFINE WINDOW MainWin ;
         ...
         ELSE
           ...
           nH := MainWin.ClientHeight - GetToolBarHeight() - GetTitleHeight() - iif( IsWin10(), GetBorderHeight(), GetBorderHeight() / 2 + 2 )
         ENDIF

         CreateBrowse( "BRW_1", GetToolBarHeight() + GetBorderHeight() / 2, 0, nW, nH )
...
Static Procedure CreateMainMenu
  ...
  DEFINE MAIN MENU OF MainWin
	...
	END POPUP 
        SEPARATOR
        ITEM 'E&xit' + Chr(9) + 'CTRL-X' ACTION { || QuickExit() }
     END POPUP
...
PROCEDURE NewTable
  ...
  IF !_HMG_DialogCancelled .AND. Len( aStruct ) > 0 .AND. !Empty( aStruct[1][1] )
    ...
    cFileDbf := PutFile( { {"File DBF (*.DBF)", "*.DBF"} }, 'New table...', GetStartupFolder() )
...
PROCEDURE NewIndex
  ...
  DEFINE WINDOW frmNewIdx;
      ...
      DEFINE EDITBOX edtKey
          ...
          HEIGHT    65
          VALUE     cKey
          ONCHANGE  cKey := AllTrim( frmNewIdx.edtKey.Value )
          NOHSCROLLBAR .T.
      END EDITBOX
...
PROCEDURE OpenDataTable( cFile )
  ...
  // Set default RDD and open a data file
  IF File( ChangeFileExt( cFile, ".fpt" ) )
    rddSetDefault( "DBFCDX" )
  ELSE
    rddSetDefault( "DBFNTX" )
  ENDIF
...
PROCEDURE NewTag
  ...
  DEFINE WINDOW frmNewTag;
      ...
      DEFINE EDITBOX edtKey
          ...
          HEIGHT    65
          VALUE     cKey
          ONCHANGE  ( lChanges := .T., cKey := AllTrim( frmNewTag.edtKey.Value ) )
          NOHSCROLLBAR .T.
      END EDITBOX
