...
STATIC TSB_SELECTOR    := .T. , aStaticLogo
/////////////////////////////////////////////////////////////////////////////
...
FUNCTION TableSetup(oBrw)   
   ...
   IF nTsbSH > 0 .OR. nTsbML > 0 .OR. nTsbH > 0 .OR. nTsbF > 0 .OR. nTsbSl > 0
...
* ======================================================================
STATIC FUNCTION ToCSharpDll(oBrw)  
   LOCAL cMaska, cPath, cXlsFile, lActivate, lSaveFile, aImage, nTime 

   oBrw:lEnabled := .F.    // блокировать область таблицы (Строки не отображаются)

   cPath      := GetStartUpFolder() + "\"      // путь записи файла
   cMaska     := "TestC#_DllXlsOle"            // шаблон файла
   cXlsFile   := cPath + cMaska + "_" + DTOC( DATE() ) + ".xls"
   cXlsFile   := GetFileNameMaskNum(cXlsFile)  // получить новое имя файла
   lActivate  := .T.                           // открыть Excel
   lSaveFile  := .T.                           // сохранить файл
   aImage     := aStaticLogo                   // файл лого для экспорта

   nTime := SECONDS()
   BrwDllXlsOle( oBrw, cXlsFile, lActivate, lSaveFile, aImage )
   TotalTimeExports("BrwDllXlsOle()=", cXlsFile, SECONDS() - nTime )  
